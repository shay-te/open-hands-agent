...
ISSUE_PLATFORM_DETAILS = {
    'youtrack': {
        'label': 'YouTrack',
        'base_url_key': 'YOUTRACK_API_BASE_URL',
        'token_key': 'YOUTRACK_API_TOKEN',
    ...
    'jira': {
        'label': 'Jira',
        'base_url_key': 'JIRA_API_BASE_URL',
    ...
    'github': {
        'label': 'GitHub Issues',
        'base_url_key': 'GITHUB_API_BASE_URL',
        'token_key': 'GITHUB_API_TOKEN',
        'owner_key': 'GITHUB_OWNER',
        'owner_label': 'repository owner or organization',
        'repo_key': 'GITHUB_REPO',
        'repo_label': 'issues repository name',
        'assignee_key': 'GITHUB_ASSIGNEE',
        'assignee_label': 'assignee login',
        'progress_state_field_key': 'GITHUB_PROGRESS_STATE_FIELD',
        'progress_state_key': 'GITHUB_PROGRESS_STATE',
        'review_state_field_key': 'GITHUB_REVIEW_STATE_FIELD',
        'review_state_key': 'GITHUB_REVIEW_STATE',
        'issue_states_key': 'GITHUB_ISSUE_STATES',
    ...
    'gitlab': {
        'label': 'GitLab Issues',
        'base_url_key': 'GITLAB_API_BASE_URL',
        'token_key': 'GITLAB_API_TOKEN',
        'project_key': 'GITLAB_PROJECT',
        'project_label': 'project path or numeric id',
        'assignee_key': 'GITLAB_ASSIGNEE',
        'assignee_label': 'assignee username',
        'progress_state_field_key': 'GITLAB_PROGRESS_STATE_FIELD',
        'progress_state_key': 'GITLAB_PROGRESS_STATE',
        'review_state_field_key': 'GITLAB_REVIEW_STATE_FIELD',
        'review_state_key': 'GITLAB_REVIEW_STATE',
        'issue_states_key': 'GITLAB_ISSUE_STATES',
    ...
    'bitbucket': {
        ...
        'base_url_key': 'BITBUCKET_API_BASE_URL',
        'token_key': 'BITBUCKET_API_TOKEN',
        'username_key': 'BITBUCKET_USERNAME',
        'email_key': 'BITBUCKET_API_EMAIL',
        'workspace_key': 'BITBUCKET_WORKSPACE',
        'workspace_label': 'workspace',
        'repo_slug_key': 'BITBUCKET_REPO_SLUG',
        'repo_slug_label': 'issues repository slug',
        'assignee_key': 'BITBUCKET_ASSIGNEE',
        'assignee_label': 'assignee username',
        'username_label': 'username for git auth',
        'api_email_label': 'email for pull request auth',
        'progress_state_field_key': 'BITBUCKET_PROGRESS_STATE_FIELD',
        'progress_state_key': 'BITBUCKET_PROGRESS_STATE',
        'review_state_field_key': 'BITBUCKET_REVIEW_STATE_FIELD',
        'review_state_key': 'BITBUCKET_REVIEW_STATE',
        'issue_states_key': 'BITBUCKET_ISSUE_STATES',
...
def _prompt_bypass_permissions(defaults: dict[str, str]) -> dict[str, str]:
    ...
    entirely. The configurator: (1) shows the danger explicitly, then
    (2) requires the operator to type the phrase ``I ACCEPT``. There is
    no second env var — at runtime, kato will additionally double-prompt
    on the terminal before starting under bypass mode (see SECURITY.md
    and BYPASS_PROTECTIONS.md).
    """
    ...
    chose_bypass = input_bool(
        ...
        'asking; refused under root; refused under non-interactive runs). When '
    ...
    if not chose_bypass:
        return {'KATO_CLAUDE_BYPASS_PERMISSIONS': _bool_to_env(False)}
    print(
        '\n'
        ...
        '!!  by the agent. Kato will double-prompt at every startup as\n'
        '!!  belt-and-suspenders. Read SECURITY.md before continuing.\n'
    )
    ...
    if confirmation.strip() != 'I ACCEPT':
        ...
        return {'KATO_CLAUDE_BYPASS_PERMISSIONS': _bool_to_env(False)}
    return {'KATO_CLAUDE_BYPASS_PERMISSIONS': _bool_to_env(True)}
