...
class KatoClientTests(unittest.TestCase):
    def test_uses_configured_retry_count(self) -> None:
        client = KatoClient('https://openhands.example', 'oh-token', max_retries=5)
        self.assertEqual(client.max_retries, 5)

    def test_uses_minimum_retry_count_of_one(self) -> None:
        client = KatoClient('https://openhands.example', 'oh-token', max_retries=0)
    ...
    def test_validate_model_access_checks_openrouter_model_availability(self) -> None:
        ...
        with patch(
            'kato.client.kato_client.OpenRouterClient.validate_model_available'
    ...
    def test_wait_for_conversation_result_uses_configured_poll_limit_in_timeout(self) -> None:
        ...
        with patch.object(
            ...
            return_value=True,
        ), patch('kato.client.kato_client.time.sleep'):
    ...
    def test_event_highlight_text_describes_shell_action(self) -> None:
        highlight = KatoClient._event_highlight_text(
            {
                'kind': 'ActionEvent',
                'source': 'agent',
                'tool_name': 'execute_bash',
    ...
    def test_parse_result_json_reads_fenced_json(self) -> None:
        payload = KatoClient._parse_result_json(
    ...
    def test_run_prompt_raises_when_conversation_reports_failed_status(self) -> None:
        ...
        with patch.object(
            ...
            with self.assertRaisesRegex(
                RuntimeError,
                'conversation failed with status: failed: recent Kato activity: ran shell command: git pull',
            ):
                implement_task_with_defaults(client)
